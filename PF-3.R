...
library("reshape2")
library("readxl")
Base1 <- read_excel("D:/Наука/Статьи и работы начатые/Ч003 - Статья с Д. В. - по отбору ПФ/Производственные функции.xlsx", sheet = 3)
glimpse(Base1)
...
model_trees_2 <- lm(data = Base1, lnY ~ lnTT+lnKT)
summary(model_trees_2)
model_trees_3 <- lm(data = Base1, lnY ~ lnTT+lnKT+lnLS) #No
...
summary(model_glm_2)
model_glm_3 <- lm(data = Base1, lnY ~ lnT+lnS+lnL) # No
summary(model_glm_3)
...
summary(model_ev_1)
model_ev_2 <- lm(data = Base1, lnY ~ lnT+lnS)
summary(model_ev_2)
model_ev_3 <- lm(data = Base1, lnY ~ lnT+lnS+lnSonT)   # No
summary(model_ev_3)
library(gvlma)
...
library(quantreg)
rq_1<-rq(data = Base1, tau = 0.5, lnY ~ lnT+lnS)
summary(rq_1)
